...
$('sensorTest').onclick=async()=>{busy=true;connected(true,'Testing WT901 sensor...');try{const j=await api('/api/sensor/test','POST');current=j;connected(!j.fault,'Sensor Test OK | az '+j.azimuth.toFixed(1)+' el '+j.elevation.toFixed(1)+' | feedback '+(j.feedback_received?(j.feedback_age_ms+' ms old'):'waiting')+' | backend '+j.motor_backend);}catch(e){connected(false,e.message)}finally{busy=false;document.querySelectorAll('[data-control]').forEach(x=>x.disabled=$('status').classList.contains('offline'))}};
