...
# validate_pi_rotator_hardening_v1_failfast_v3.sh
#
# Fail-fast PASS/FAIL validator for PI-SATELLITE-ROTATOR hardening-status-feedback.
#
# This v3 validator avoids brittle checks for exact web endpoint source text.
# Web endpoint behavior is validated by the Docker/CMake native test suite.
#
# Run from repo root:
#
#   cd ~/sdrdev/PI-SATELLITE-ROTATOR
#   ./tools/validate_pi_rotator_hardening_v1_failfast_v3.sh
...
PY
  then
    ...
    printf 'FAIL: %s\n' "$description" >&2
    printf '      file: %s\n' "$file" >&2
    printf '      expected all required source markers.\n' >&2
    echo "FINAL RESULT: FAIL" >&2
    exit 1
  fi
}

echo "PI-SATELLITE-ROTATOR hardening v1 fail-fast validation v3"
echo "========================================================="
...
require_file_contains_any "tests/test_web.cpp"   "web tests cover status JSON behavior"   "feedback_stale"   "moving"   "backend"
...
if ./scripts/test.sh; then
  pass "Docker native CMake tests including web endpoint behavior"
else
  fail "Docker native CMake tests including web endpoint behavior"
fi
