...
require_dir() {
  ...
  if [[ -d "$dir" ]]; then
    ...
    fail "$description - missing directory: $dir"
  fi
}
...
require_clean_source_check_prereqs() {
...
echo "PI-SATELLITE-ROTATOR hardening v1 fail-fast validation"
echo "======================================================"
...
require_file_contains_any "src/rotator.cpp"   "stale feedback detection exists"   "feedback_stale_locked"

require_file_contains_any "src/rotator.cpp"   "motion is rejected before first live feedback sample"   "external feedback has not produced a valid sample"   "feedback has not produced a valid sample"   "obtain valid feedback"

require_file_contains_any "src/rotator.cpp"   "motion is rejected when feedback is stale"   "external feedback is stale"   "feedback is stale"
...
echo
echo "Checking git working tree has changes to commit..."
